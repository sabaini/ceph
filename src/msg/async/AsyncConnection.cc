...
void *AsyncConnection::DelayedDelivery::entry()
{
  ...
  while (!stop_delayed_delivery) {
    ...
    Message *m = delay_queue.front().second;
    string delay_msg_type = pipe->async_msgr->cct->_conf->ms_inject_delay_msg_type;
    if (!flush_count &&
        (release > ceph_clock_now(pipe->async_msgr->cct) &&
    ...
    if (pipe->async_msgr->ms_can_fast_dispatch(m)) {
      if (!stop_fast_dispatching_flag) {
        delay_dispatching = true;
        delay_lock.Unlock();
        pipe->async_msgr->ms_fast_dispatch(m);
        delay_lock.Lock();
      ...
      pipe->center->dispatch_event_external(EventCallbackRef(new C_handle_dispatch(pipe->async_msgr, m)));
    }
    active_flush = false;
  }
  return NULL;
